// ============================================================
// BORG-ADMIN - Backend Admin IA Worker
// ============================================================
// Stack: TypeScript Strict, grammY, Cloudflare Workers, D1
...
async function wakeCodespace(token: string): Promise<{ success: boolean; state: string }> {
  ...
  if (!response.ok) return { success: false, state: 'error' };
...
async function callLlama(llamaUrl: string, prompt: string): Promise<LlamaResponse> {
  ...
  if (!response.ok) throw new Error(`llama-server: ${response.status}`);
...
async function endSession(db: D1Database, chatId: number): Promise<void> {
  await db.prepare(`UPDATE chat_sessions SET active = 0 WHERE chat_id = ?`).bind(chatId).run();
}
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
      ...
      if (model === 'local') {
        ...
        try {
          ...
          if (csStatus.state === 'Shutdown' || csStatus.state === 'Unavailable') {
            await ctx.api.editMessageText(chatId, statusMsg.message_id, "⏳ Codespace dormido. Reactivando... (~2 min)");
            const wakeResult = await wakeCodespace(env.GH_PAT);
            if (!wakeResult.success) {
              await ctx.api.editMessageText(chatId, statusMsg.message_id, "❌ Error al reactivar. Usa '⚡ Wake Codespace'");
              return;
            }
            await ctx.api.editMessageText(chatId, statusMsg.message_id, "⏳ Codespace activo. Esperando LLM...");
            let llmReady = false;
            for (let i = 0; i < 36; i++) {
              await new Promise(r => setTimeout(r, 5000));
              if (await checkLLMHealth(env.LLAMA_URL)) { llmReady = true; break; }
            }
            if (!llmReady) {
              await ctx.api.editMessageText(chatId, statusMsg.message_id, "⚠️ LLM no responde. Ejecuta ./start-frankenstein.sh en Codespace.");
              return;
            }
          }
          const llmOk = await checkLLMHealth(env.LLAMA_URL);
          if (!llmOk) {
            await ctx.api.editMessageText(chatId, statusMsg.message_id, "⚠️ LLM no responde. Ejecuta ./start-frankenstein.sh");
            return;
          }
        } catch (e) {
          await ctx.api.editMessageText(chatId, statusMsg.message_id, `❌ Error: ${e instanceof Error ? e.message : 'desconocido'}`);
          return;
        }
        await createSession(env.DB, chatId, 'local');
        await ctx.api.editMessageText(chatId, statusMsg.message_id, "🖥️ **LLM Local Conectado**\n\n_Qwen3.5-4B-Uncensored_\n\nEscribe tu mensaje. Usa /start para volver.", { parse_mode: "Markdown" });
      } else if (model === 'gemini') {
        await createSession(env.DB, chatId, 'gemini');
        await ctx.editMessageText("🌐 **Gemini Conectado**\n\n_Gemini 2.0 Flash Exp_\n\nEscribe tu mensaje. Usa /start para volver.", { parse_mode: "Markdown" });
      }
    });
    ...
    bot.callbackQuery("action:status", async (ctx) => {
      ...
      try {
        ...
        csStatus = cs.state === 'Available' ? '🟢 Activo' : cs.state === 'Shutdown' ? '🔴 Dormido' : `🟡 ${cs.state}`;
    ...
    bot.callbackQuery("action:wake", async (ctx) => {
      ...
      try {
        ...
        if (result.success) {
          await ctx.editMessageText("⚡ **Codespace Reactivado**\n\n" + `Estado: ${result.state}\n\n` + "Espera ~2 min antes de usar el LLM Local.", { parse_mode: "Markdown" });
        } else {
          await ctx.editMessageText("❌ Error al reactivar. Verifica GH_PAT.", { parse_mode: "Markdown" });
        }
      } catch (e) {
        await ctx.editMessageText(`❌ Error: ${e instanceof Error ? e.message : 'desconocido'}`, { parse_mode: "Markdown" });
      }
    });
    ...
    bot.on("message:text", async (ctx) => {
      ...
      if (!session || session.active !== 1) {
        await ctx.reply("⚠️ No tienes sesión activa.\n\nUsa /start para seleccionar modelo.", { parse_mode: "Markdown" });
        return;
      }
      ...
      if (session.model === 'gemini') {
        ...
        const llmOk = await checkLLMHealth(env.LLAMA_URL);
        if (!llmOk) {
          await ctx.api.editMessageText(chatId, statusMsg.message_id, "⏳ LLM no disponible. Verificando...");
          const csStatus = await getCodespaceStatus(env.GH_PAT);
    ...
    bot.command("history", async (ctx) => {
      ...
      const formatted = history.map(h => `**${h.role === 'user' ? '👤 Tú' : '🤖 IA'}**: ${h.content.substring(0, 100)}...`).join('\n\n');
